...
def plot(dataSets, category, field, y_range, autoscale, dry, target_dir)
  ...
  Gnuplot.open do |gp|
    Gnuplot::Plot.new(gp) do |plot|
      ...
      plot.yrange "[0:#{y_range}]"
      if autoscale then plot.set "autoscale" end
...
def read_options_and_arguments
  ...
  optparse = OptionParser.new do |opts|
    ...
    options[:y_range] = 105
    opts.on('-y', '--y-range RANGE', 'Sets the range of the y-axis') do |range|
      options[:y_range] = range
    end
...
plot(dataSets, options[:category], options[:field], options[:y_range], false, options[:dry], options[:target_dir])
