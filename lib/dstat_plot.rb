...
def data_preprocessing(dataset_container, slice_size)
  dataset_container[:datasets].each do |dataset|
    ...
    values = dataset.data[1].map(&:to_f)
...
def translate_to_column(category, field, csv)
  ...
  puts "'#{category}-#{field}' was translated to #{category_index + field_index}." if $verbose
  category_index + field_index
end
...
def read_data_from_csv(files, category, field, column, no_plot_key, y_max, inversion, title, smooth)
  ...
  files.each do |file|
    ...
    puts "Reading from csv to get #{prefix}." if $verbose
    ...
    csv = csv.drop(7) if csv[2].index 'Host:'
    ...
    if y_max.nil?
      local_maximum = values.max_by(&:to_f).to_f
      overall_max = local_maximum if local_maximum > overall_max
    end
...
def read_options_and_arguments
  ...
  optparse = OptionParser.new do |parser|
    ...
    parser.on('-l', '--column COLUMN', 'Select the desired column directly.', "\n") do |column|
      # -c and -f override -l
      opts[:column] = column.to_i unless opts[:category] && opts[:field]
    end
  ...
  optparse.parse!
  puts "opts: #{opts.inspect}" if $verbose
  ...
  opts[:files] = files
  puts "files: #{files.count} #{files.inspect}" if $verbose
...
class DstatPlot
  def self.run
    ...
    dataset_container = read_data_from_csv(
      opts[:files],
      opts[:category],
      opts[:field],
      opts[:column],
